    ...
    <Transition.Root show={isDialogOpen} as={Fragment}>
      ...
      <div className="fixed inset-y-0"></div>
        ...
        <div className="visible sm:invisible fixed inset-0">
                ...
                <Dialog.Panel className="pointer-events-auto absolute inset-y-0 max-w-full inset-x-0 overflow-y-scroll">
                  <div className="flex w-fit h-full flex-col bg-white py-6 rounded-lg shadow-xl">
                    ...
                    <div className="relative inset-x-0">
                          <Image src={`https://patchwork-kingdoms.fra1.digitaloceanspaces.com/thumbnail/${data?.cluster_id}.png`} layout="responsive" width="100%" height="100%" objectFit='contain'/>
                          <div className="bg-teal-600 px-8 pt-6 pb-8 rounded-b-lg shadow-b-xl">
          ...
          {/* first transition if only half closed 
          <div className=' fixed inset-0 max-w-full max-h-full overflow-clip'>
            <div className='pl-4 pt-24'>   
            <div className=''>
              <Transition.Child  
                  ...
                  leaveFrom='translate-y-0 opacity-100'
                  leaveTo='translate-y-1/2 opacity-100'
                >
                <Dialog.Panel className="pointer-events-auto absolute inset-y-0 max-w-full inset-x-0">
                  <div className="flex w-fit h-1/2 flex-col bg-white py-6 rounded-lg shadow-xl">
                    <div className="px-4 sm:px-8">
                      <div className="flex items-start justify-between border-b-2 border-grey-100 pb-2">
                        <Dialog.Title className="text-xl font-bold text-teal-600"> {"#" + data?.name.split('#')[1]} </Dialog.Title>
                        <button
            ...
            {/* second transition if only half closed 
            <div>
              <Transition.Child  
                  as={Fragment}
                  enter="transition transform ease-out duration-0"
                  enterFrom="translate-y-full opacity-0"
                  enterTo="translate-y-0 opacity-100"
                  leave='transition transform ease-in duration-0'
                  leaveFrom='translate-y-0 opacity-100'
                  leaveTo='translate-y-full opacity-00'
                >
                <Dialog.Panel className="pointer-events-auto absolute inset-y-1/2 max-w-full inset-x-0">
                  <div className="flex w-fit h-1/2 flex-col bg-white py-6 rounded-lg shadow-xl">
                    <div className="relative mt-4 flex-1 px-4 sm:px-6">
                      <div className="absolute inset-x-0">
                          <Image src={`https://patchwork-kingdoms.fra1.digitaloceanspaces.com/thumbnail/${data?.cluster_id}.png`} layout="responsive" height="0" width="50" objectFit='contain'/>
                          <div className="bg-teal-600 px-8 pt-6 pb-8 rounded-b-lg shadow-b-xl">
                                <h2 className="text-white font-bold text-2xl leading-7" >Connecting every school in the world to the internet</h2>
                                <p className="text-white font-medium text-sm leading-5 mt-2">
                                    Each Patchwork Kingdom tells a unique story on school connectivity. The artwork is data generated and based on the connectivity status of real schools spread all over the world. 
